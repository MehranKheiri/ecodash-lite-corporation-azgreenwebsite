// Fixed social sidebar - visible on desktop, hidden on mobile (shown in footer instead)
const socials = [
  {
    name: 'LinkedIn',
    ...
    <aside className="social-sidebar" aria-label="Social media links">
      {socials.map(s => (
        <a
          ...
          title={s.name}
        >
          <span className="social-sidebar-label">{s.name}</span>
          <span className="social-sidebar-icon">{s.icon}</span>
        </a>
      ))}
    </aside>
  )
